...
class Solver2 implements Runnable {
    ...
    /**
     * An array of all the spaces in the <code>puzzle</code>. After this array is initialized,
     * each entry references a space in the <code>puzzle</code>. If <code>spaces[index]</code> references
     * <code>puzzle[row][column]</code>, then <code>spaces[index] == 9 * row + column</code>. Vice versa,
     * the row and column of the space that <code>spaces[index]</code> references are
     * <ul>
     * <li> <code>row == spaces[index] / 9</code>, and
     * <li> <code>column == spaces[index] % 9</code>.
     * </ul>
     * Each space in the <code>puzzle</code> is referenced exactly once. 
     * Filled spaces preceed empty spaces, but otherwise there is no particular
     * order to entries in <code>spaces</code>.
     */
     ...
     */
    private int index = 0;
    // A random number genrator used to shuffle the spaces array.
    ...
    public void run() {
        ...
        System.out.format("Number of iterations: %d%n"
                + "Time elapsed: %d ms%n", iterations, System.currentTimeMillis() - startTime);
        done = true;
    ...
    private void initializeSpaces() {
        ...
        for (int k = 0; k < spaces.length; k++) {
            if (puzzle[spaces[k] / 9][spaces[k] % 9] != 0) { //clue found
                // swap the place containing a clue to the front of the spaces array
     ...
     * solution.
     * </ol>
     * Calling <code>findEmptySpace()</code> has no effect on the
     * correctness of the algorithm, but may have significant effect on the
     * processing time.
     *
     */
    ...
    private void findEmptySpace() {
        ...
        if (minIndex > index) { // swap empty space to the front of empty spaces.
            int p = spaces[index];
